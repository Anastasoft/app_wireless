...
public class Sockettest extends Activity {
	...
	public boolean Connect() {
		...
		try {//creamos sockets con los valores anteriores
			...
			OutputStream out = miCliente.getOutputStream();
			InputStream input = miCliente.getInputStream();
			in = new BufferedReader(new InputStreamReader(input));

			oos = new PrintStream(out);
			Log.e("Conectou","!!!!");
	...
	public boolean Snd_Msg(Mensaje_data msg) {
		...
		try {
			...
			if (miCliente.isConnected())// si la conexion continua
			{
				...
				String msg_string = mensaje.texto;
				oos.println(msg_string);
				in.readLine();
				//envio ok
				return true;
